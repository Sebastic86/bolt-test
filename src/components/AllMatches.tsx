...
const AllMatches: React.FC<AllMatchesProps> = ({
  ...
  return (
    <div className="w-full max-w-4xl mt-8">
      ...
      {loading && allMatches.length === 0 && <p className="text-center text-gray-600">Loading matches...</p>}
      {error && <p className="text-center text-red-600 bg-red-100 p-3 rounded-sm">{error}</p>}
      {!loading && !error && allMatches.length === 0 && (
        <p className="text-center text-gray-500">No matches recorded.</p>
      )}
              ...
              <li key={match.id} className={`rounded-lg shadow-sm p-4 border transition-all duration-200 ${highlightClasses} ${isDeletingThisMatch ? 'opacity-50' : ''}`}>
                ...
                {expandedMatchId === match.id && (
                 ...
                 <div className="text-right text-xs text-gray-400 mt-1">
                   Played: {formatDateTimeEuropean(match.played_at)}
                 </div>
              </li>
            );
          })}
