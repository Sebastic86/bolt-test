...
const TopWinPercentageTeams: React.FC<TopWinPercentageTeamsProps> = ({ 
  ...
  useEffect(() => {
    const fetchVersions = async () => {
      ...
      try {
        ...
        console.error('[TopWinPercentageTeams] Error fetching versions:', err);
  ...
  const computedStandings: TeamStanding[] = useMemo(() => {
    ...
    filteredMatches.forEach((match) => {
      ...
      if (!hasScores) return; // Only consider completed matches
  ...
  const topTeams = computedStandings
